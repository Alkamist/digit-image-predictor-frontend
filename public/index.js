...
class PredictorModule {
	...
	async load(wasmPath, intSize = 4) {
		if (this.loaded) return;

		this.wasmMemoryInterface = new odin.WasmMemoryInterface();
		this.wasmMemoryInterface.setIntSize(intSize);

		let imports = odin.setupDefaultImports(this.wasmMemoryInterface, null, this.wasmMemoryInterface.memory);

		const response = await fetch(wasmPath);
		const file = await response.arrayBuffer();
		const wasm = await WebAssembly.instantiate(file, imports);
		this.exports = wasm.instance.exports;
		this.wasmMemoryInterface.setExports(this.exports);

		if (this.exports.memory) {
			if (this.wasmMemoryInterface.memory) {
				console.warn("WASM module exports memory, but interface has existing memory too");
			}
			this.wasmMemoryInterface.setMemory(this.exports.memory);
		}
	...
	predict(imageData) {
		...
		const mnistImageSize = 784;
		const imageDataPtr = this.wasmMemoryInterface.loadF32Array(
			this.exports.get_image_data_pointer(),
			mnistImageSize
		);

		for (let i = 0; i < mnistImageSize; i++) {
			imageDataPtr[i] = imageData[i];
		}

		return this.exports.predict();
	}
}

const predictorModule = new PredictorModule();
...
function MNISTDrawer() {
	...
	useEffect(() => {
		const loadWasm = async () => {
			try {
				setIsLoading(true);
				await predictorModule.load("main.wasm", 4);
				setWasmLoaded(true);
				setError(null);
			} catch (err) {
	...
	const predictWithWasm = () => {
		if (!wasmLoaded) {
			setError("WASM module is still loading...");
			return;
		}
		...
		try {
			...
			const prediction = predictorModule.predict(imageData);
